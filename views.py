...
def load_menu_from_file(file_path: str) -> List[Dict[str, str]]:
    try:
...
products: List[Dict[str, str]] = load_menu_from_file(name_js_file)
...
def get_product_details(product_name: str) -> jsonify:
...
def get_product_info_by_field(product_name: str, product_field: str) -> jsonify:
    product_name = unquote(product_name)
    product = get_product_by_name(product_name)
